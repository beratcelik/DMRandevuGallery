...
/// yalnızca bu videoyu kayıttan ÇIKARIYOR ve ihbar kalan delille memurda
/// kalmaya devam ediyor. Tek bir metin, gerçekleşmeyecek bir şeyi vaat ederdi.
enum IhbarRetractCopy {

    /// Kayıtta bu videodan başka, HENÜZ ELENMEMİŞ video kalıyor mu?
    ///
    /// Sayı gelmediyse (eski sunucu) geri çekme metni kalıyor: daha ağır olanı
    /// vaat etmek, hafif olanı vaat edip ağırını yapmaktan iyi.
    static func detachOnly(_ mark: IhbarMark) -> Bool {
        mark.mediaCount - mark.eliminatedCount - 1 > 0
    }

    static func title(_ mark: IhbarMark) -> String {
        detachOnly(mark) ? Strings.ihbarRetractDetachTitle : Strings.ihbarRetractTitle
    }

    static func explain(_ mark: IhbarMark) -> String {
        detachOnly(mark)
            ? Strings.ihbarRetractDetachExplain(mark.mediaCount)
            : Strings.ihbarRetractExplain
    }

    static func confirm(_ mark: IhbarMark) -> String {
        detachOnly(mark) ? Strings.ihbarRetractDetachConfirm : Strings.ihbarRetractConfirm
    }
}
